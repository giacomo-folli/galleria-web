			...
			if (open && localOpen != open) {
				...
				if (otherDialogs.length > 0) {
					otherDialogs.forEach((dialog) => {
						...
						if (!maxZIndex || maxZIndex < Number(currentZIndex)) maxZIndex = Number(currentZIndex);
					});
					zIndex = maxZIndex + 2;
				}

				let otherDrawers: NodeListOf<HTMLElement> = document.querySelectorAll('[data-drawer=true]');
...
<style>
	...
	.overlay-container-active {
		animation: z-index-showing var(--dialog-transition-timing-function)
			calc(var(--dialog-transition-duration) - 0.2s) forwards;
	}

	.overlay-container-deactive {
		animation: z-index-hiding var(--dialog-transition-timing-function)
			calc(var(--dialog-transition-duration) - 0.2s) forwards;
	}
	...
	.fly-down-in {
		animation: fly-down-in-kf var(--dialog-transition-timing-function)
			var(--dialog-transition-duration) forwards;
	}

	.fly-down-out {
		animation: fly-down-out-kf var(--dialog-transition-timing-function)
			var(--dialog-transition-duration) forwards;
	}
	...
	.fly-up-in {
		animation: fly-up-in-kf var(--dialog-transition-timing-function)
			var(--dialog-transition-duration) forwards;
	}

	.fly-up-out {
		animation: fly-up-out-kf var(--dialog-transition-timing-function)
			var(--dialog-transition-duration) forwards;
	}
	...
	.fly-horizontal-in {
		animation: fly-horizontal-in-kf var(--dialog-transition-timing-function)
			var(--dialog-transition-duration) forwards;
	}

	.fly-horizontal-out {
		animation: fly-horizontal-out-kf var(--dialog-transition-timing-function)
			var(--dialog-transition-duration) forwards;
	}
	...
	.scale-in {
		animation: scale-in-kf var(--dialog-transition-timing-function)
			var(--dialog-transition-duration) forwards;
	}

	.scale-out {
		animation: scale-out-kf var(--dialog-transition-timing-function)
			var(--dialog-transition-duration) forwards;
	}
	...
	.fade-in {
		animation: fade-in-kf var(--dialog-transition-timing-function) var(--dialog-transition-duration)
			forwards;
	}

	.fade-out {
		animation: fade-out-kf var(--dialog-transition-timing-function)
			var(--dialog-transition-duration) forwards;
	}
